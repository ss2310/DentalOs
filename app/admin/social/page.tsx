...
export default async function AdminSocialPage() {
  ...
  const [{ data: posts }, { data: clinics }, { data: ledger }] = await Promise.all([
    db
      .from("social_posts")
      .select("clinic_id, status, credits_deducted, created_at")
  ...
  const rowFor = (id: string) => {
    ...
    if (!r) {
      r = { clinic: names.get(id) ?? id.slice(0, 8), generated: 0, approved: 0, posted: 0, credits: 0 };
      byClinic.set(id, r);
    }
    return r;
  };
  for (const p of (posts ?? []) as { clinic_id: string; status: string }[]) {
